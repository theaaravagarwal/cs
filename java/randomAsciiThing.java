...
public class randomAsciiThing {
    ...
    public static void main(String[] args) {
        ...
        button.addActionListener(e -> new Thread(() -> chooseAndConvertImage()).start()); // Load in a separate thread
        ...
        if (fileName.endsWith(".gif")) {
            ...
            try (ImageInputStream in = ImageIO.createImageInputStream(file)) {
                ...
                for (int i = 0; i < numImages; i++) {
                    ...
                    frames.add(frame); // Directly add the frame without creating a new BufferedImage
                }
            }
            reader.dispose();
        } else {
            ...
            if (image != null) {
                frames.add(image); // Directly add the image
            }
        }
    ...
    private static void displayColoredAsciiGif(List<String> asciiFrames, int imageWidth, int imageHeight) {
        ...
        Timer timer = new Timer(100, null); // Adjusted delay to 100ms for smoother performance
