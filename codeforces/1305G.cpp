...
int main() {
    ...
    while (cmp > 1) {
        ...
        for (int j=0; j<=n; j++) {
            ...
            else if (best[a[j]][1]<f&&best[a[j]][0].second!=f.second) best[a[j]][1] = f;
        }
            ...
            for (int mask=0; mask<(1<<18); mask++) {
                if (mask&(1<<bit)) {
                    for (int iter=0; iter<2; iter++) {
                        ...
                        else if (best[mask][1]<best[mask^(1<<bit)][iter] && best[mask][0].second!=best[mask^(1<<bit)][iter].second) best[mask][1] = best[mask ^ (1 << bit)][iter];
                    }
                }
            }
        }
